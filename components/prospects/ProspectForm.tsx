...
export default function ProspectForm({
  ...
  const assignedEmployeeId = prospect?.assignedToId
  ...
  const { data: assignStates = [] } = useStates(showAssignPicker);
  const { data: assignBranches = [] } = useBranches(
    { stateId: assignStateId || undefined },
    showAssignPicker && !!assignStateId,
  );
  const { employees, isLoading: employeesLoading } = useSalesEmployees({
    ...
    enabled: showAssignPicker && assignScopeReady,
  });
  ...
  }, [mode, setValue, getValues, generatePasswordFN]);
  ...
  // Fallback: derive state/branch from assigned employee detail
  useEffect(() => {
    if (!showAssignPicker || !assignedEmployee) return;
    if (!assignStateId && assignedEmployee.stateId) {
      setAssignStateId(String(assignedEmployee.stateId));
    }
    if (!assignBranchId && assignedEmployee.branchId) {
      setAssignBranchId(String(assignedEmployee.branchId));
    }
  }, [showAssignPicker, assignedEmployee, assignStateId, assignBranchId]);
  ...
  const employeeOptions = useMemo(() => {
    ...
    const currentId = watchedAssignedToId || assignedEmployeeId;
    if (
      currentId &&
                ...
                <div className="col-span-full md:col-span-1">
                  <div className="flex flex-col gap-1">
                    ...
                    <select
                      value={assignStateId}
                      onChange={(e) => {
                        setAssignStateId(e.target.value);
                        setAssignBranchId("");
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Select
                        ...
                        name={field.name}
                        ref={field.ref}
                        disabled={!assignScopeReady || employeesLoading}
                      />
                    )}
                  />
                </div>
